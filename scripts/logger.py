...
class LoggerBase(object):
    ...
    def sanitizeLog(self, logdata):
        ...
        logdata['local_time'] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
class PyLogger(LoggerBase):
    ...
    def log(self, logdata, retry=True):
        ...
        msg = msg + " - host: " + str(logdata['src_host'])
        msg = msg + " - port: " + str(logdata['dst_port'])
        msg = msg + " - logtype: " + str(logdata['logtype'])
        msg = msg + " - node: " + str(logdata['node_id'])
        msg = msg + " - details: " + str(json.dumps(logdata['logdata'], sort_keys=True))
        #msg = msg + " - " + json.dumps(logdata, sort_keys=True)
        self.logger.warn(msg)
